...
train_for_survival<-select(train_for_survival,c(3:11,13))
testset_for_survival<-select(testset_for_survival,c(3:11,13))
...
myvars<-c("Label", "tumor_size", "Age", "surging", "garde", "maritalstatus", 
          "Race", "vascular_invasion", "Survival_months", "Stage", "type")

## 需要转为分类变量的变量
catVars <- c("Label","surging", "garde", "maritalstatus", 
             "Race", "vascular_invasion","Stage", "type")
